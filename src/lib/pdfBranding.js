...
// top of a jsPDF document. Falls back gracefully to plain "KoraBiz" text
...
export async function drawBrandedHeader(doc, profile, startX = 14) {
  ...
  if (logoDataUri) {
    ...
    doc.text(profile?.company_name || 'KoraBiz Management System', startX + 26, y + 9)
    ...
    doc.setFontSize(14)
    doc.setFont(undefined, 'bold')
    doc.text(profile?.company_name || 'KoraBiz Management System', startX, y)
    doc.setFont(undefined, 'normal')
    doc.setFontSize(9)
    let subY = y + 6
    if (profile?.company_location) {
...
export async function drawBrandedHeaderNarrow(doc, profile) {
  ...
  doc.setFontSize(12)
  doc.setFont(undefined, 'bold')
  doc.text(profile?.company_name || 'KoraBiz Management System', centerX, y, { align: 'center' })
  doc.setFont(undefined, 'normal')
  y += 5
