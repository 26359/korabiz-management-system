...
export default function Credits() {
  ...
  const handleExportClientPDF = async () => {
    ...
    doc.save(`KoraBiz_${label}_${selectedCustomer.name.replace(/\s+/g, '_')}_Credits.pdf`)
  }
  ...
  const handleExport = () => {
    ...
    if (exportType === 'excel') {
      ...
      XLSX.writeFile(wb, `KoraBiz_${label}_${exportFrom || 'all'}_to_${exportTo || 'all'}.xlsx`)
    } else {
      const doc = new jsPDF()
      doc.setFontSize(16)
      ...
      doc.save(`KoraBiz_${label}_${exportFrom || 'all'}_to_${exportTo || 'all'}.pdf`)
    }
